  ...
  describe('Result calculation', () => {
    ...
    it('calculates ranking', () => {
      ...
      expect(results[0]).toEqual(
        expect.objectContaining({ name: 'Referenzzeit', rank: '-' }),
      );
      expect(results[1]).toEqual(
        expect.objectContaining({ name: 'Michael', rank: 1 }),
      );
      expect(results[2]).toEqual(
        expect.objectContaining({ name: 'Peter', rank: 2 }),
      );
      expect(results[3]).toEqual(
        expect.objectContaining({ name: 'Paul', rank: 3 }),
      );
    });
    ...
    it('calculates delay to previous and to first', () => {
      ...
      expect(results).toHaveLength(4);
      expect(results[0]).toEqual(
        expect.objectContaining({ name: 'Referenzzeit', delay: '---' }),
      );
      expect(results[1]).toEqual(
    ...
    it('gives same rank to same time', () => {
      ...
      expect(results[1]).toEqual(
        expect.objectContaining({ name: 'Paul', rank: 1 }),
      );
      expect(results[2]).toEqual(
        expect.objectContaining({ name: 'Greg', rank: 2 }),
      );
      expect(results[3]).toEqual(
        expect.objectContaining({
          name: 'Peter',
